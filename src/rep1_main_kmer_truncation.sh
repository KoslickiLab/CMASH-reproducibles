...
###### Setup variables
date 
while getopts q:r:k:c:g:d:h opts
do
	case "$opts" in
		...
		g) repo="$OPTARG";;			# absolute path to the github repo
...
-g: absolute path to the github repo "CMASH-reproducibles"
...
bash ${repo}/src/rep1_meta_vs_ref_CI_compare.sh -q bb_meta_path.txt -r ref_path.txt -k 60 -c 5-60-5 -g ${repo}/src/CMash -d ${conda_path} -e ${cmash}
bash ${repo}/src/rep1_meta_vs_ref_CI_compare.sh -q cami_meta_path.txt -r ref_path.txt -k 60 -c 5-60-5 -g ${repo}/src/CMash -d ${conda_path} -e ${cmash}
